...
void test_contactlist_operations(void) {
    ...
    Contact* c1 = contactlist_add(&list);
    strcpy(c1->last_name, "Петров");
    strcpy(c1->first_name, "Петр");

    Contact* c2 = contactlist_add(&list);
    strcpy(c2->last_name, "Сидоров");
    strcpy(c2->first_name, "Алексей");

    assert(list.count == 2);
    assert(strcmp(list.items[0].last_name, "Петров") == 0);
    assert(strcmp(list.items[1].last_name, "Сидоров") == 0);

    contactlist_remove(&list, 0);
    
    assert(list.count == 1);
    assert(strcmp(list.items[0].last_name, "Сидоров") == 0);
